...
import { createContext, useContext, useState, useEffect, type ReactNode } from "react" // ✅ FIX 1
import { apiService } from "../services/api"
import { useAuth } from "./AuthContext"  // ✅ FIX 2 (removed User)
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {  // ✅ FIX 1 applied here
// ⚠️ Ensure the provider name matches the requested DataProvider
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        console.log(
          "[v0] API Base URL:",
          import.meta.env.VITE_API_URL || "https://gateplatform.onrender.com/api"
        );
...
// 🛑 DEBUG LINE ADDED HERE 🛑
  ...
  const addPdfToChapter = async (
    ...
    ? response.data.url
    : `${import.meta.env.VITE_API_URL_WITHOUT_API || "https://gateplatform.onrender.com"}${response.data.url}`,
};
